...
def memo(a,b,nums1,nums2,dp):
    if a<0 or b<0:
        return 0
    if dp[a][b]!=-1:
        return dp[a][b]
    elif nums1[a] == nums2[b]:
        dp[a][b] = 1 + memo(a-1,b-1,nums1,nums2,dp)
    else:
        dp[a][b] = max(memo(a-1,b,nums1,nums2,dp),memo(a,b-1,nums1,nums2,dp))
    return dp[a][b]
...
return memo(len(nums1)-1,len(nums2)-1,nums1,nums2,dp)
