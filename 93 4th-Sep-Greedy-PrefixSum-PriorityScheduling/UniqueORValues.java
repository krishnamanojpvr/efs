...
public class UniqueORValues{
    // public static void backtrack(int[] arr, Set<Integer> set, int index, int or, int n){
    //     for(int i = index;i<n;i++){
    //         int newOr = or | arr[i];
    //         set.add(newOr);
    //         backtrack(arr,set,index+1,newOr,n);
    //     }
    // }
    public static int getUniqueAnswers(int[] arr, int n){
        Set<Integer> set = new HashSet<>();
        // backtrack(arr,set,0,0,n);
        for(int i = 0;i<n;i++){
            int currentOr = 0;
            for(int j = i;j<n;j++){
                currentOr |= arr[j];
    ...
    public static void main(String[] args){
        ...
        System.out.println(getUniqueAnswers(arr,n));
        sc.close();
    }
}